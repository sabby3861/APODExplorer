...
struct APODRepositoryTests {
    ...
    @Test("Network failure with mismatched cached date still serves cache")
    func networkFailureWithMismatchedCacheServesCache() async throws {
        // Cache holds an APOD from a different day than the one being
        // requested. Brief-compliant behaviour: "last service call should
        // be cached and loaded if any subsequent service call fails." We
        // serve whatever cache we have rather than nothing — the offline
        // badge in the UI signals to the user that this isn't fresh.
        ...
        #expect(result.source == .cache)
        #expect(result.apod == cachedFixture)
    }
